...
def add_transaction():
    if request.method == "POST":
        ...
        # MONGODB TRANSACTION — atomic insert + optional order update
        mongo_session = client.start_session()
        try:
            with mongo_session.start_transaction():
                try:
                    transactions_col.insert_one(txn_doc, session=mongo_session)
                except pymongo_errors.DuplicateKeyError:
                ...
                # If payment is completed, atomically mark linked order as confirmed
            ...
            logger.error(f"MongoDB transaction failed, rolling back: {e}")
            flash("Transaction failed due to a database error. Please try again.", "error")
        finally:
...
def edit_transaction(id):
    ...
    if request.method == "POST":
        ...
        # MONGODB TRANSACTION — atomic update + order sync
        mongo_session = client.start_session()
        try:
            with mongo_session.start_transaction():
                transactions_col.update_one({"_id": oid}, {"$set": updates}, session=mongo_session)
            ...
            flash("Update failed due to a database error.", "error")
        finally:
            mongo_session.end_session()
        return redirect(url_for("transactions"))
...
def add_employee():
    if request.method == "POST":
        ...
        # MONGODB TRANSACTION — user account + employee record created atomically
        mongo_session = client.start_session()
        try:
            with mongo_session.start_transaction():
            ...
            flash("Could not create employee. Please try again.", "error")
        finally:
            mongo_session.end_session()
...
def delete_employee(id):
    ...
    employee = employees_col.find_one({"_id": oid})
    if employee:
        # MONGODB TRANSACTION — delete employee + login atomically
        mongo_session = client.start_session()
        try:
            with mongo_session.start_transaction():
                users_col.delete_one({"username": employee.get("username")}, session=mongo_session)
                employees_col.delete_one({"_id": oid}, session=mongo_session)
            flash("Employee and login account deleted.", "success")
        except pymongo_errors.PyMongoError as e:
            logger.error(f"Employee delete failed: {e}")
            flash("Could not delete employee. Please try again.", "error")
        finally:
            mongo_session.end_session()
    return redirect(url_for("employees"))
